...
btnExpense.addEventListener('click', async e => {
  ...
  document.querySelector('#layoutSidenav').appendChild(background);
...
async function getContent() {
  const html = await fetch('http://localhost/ProyectoMVC_Parte3/expenses/create').then(res => res.text());
  return html;
}
...
async function drawChart() {
  const http = await fetch('http://localhost/ProyectoMVC_Parte3/expenses/getExpensesJSON')
    .then(json => json.json())
    .then(res => res);
